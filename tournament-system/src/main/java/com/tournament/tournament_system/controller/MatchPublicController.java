...
import com.tournament.tournament_system.dto.BracketMatchResponse;
...
public class MatchPublicController {
    ...
    public ResponseEntity<?> getTournamentBracket(@PathVariable Integer tournamentId) {
        List<Match> matches = matchRepository.findByTournamentId(tournamentId).stream()
                .filter(Match::getIsBracketMatch)
                .sorted(Comparator.comparingInt(Match::getRoundNumber))
                .toList();

        List<BracketMatchResponse> response = matches.stream().map(match -> {
            BracketMatchResponse dto = new BracketMatchResponse();
            dto.setMatchId(match.getId());
